...
6. 生成 XML。
   - 只基于已确认的 Mermaid 草图和参考资料中的语法生成 XML。
...
7. 交付前校验。
   ...
   - 如果可以访问本技能的 `scripts/validate_soldata_xml.py`，运行 `python3 scripts/validate_soldata_xml.py <xml-file>`；否则进行人工结构校验。
...
```xml
<Tool Id="t0#s" Type="RootStart" Name="Start">...</Tool>
<Tool Id="t0#e" Type="RootEnd" Name="End">...</Tool>
```
...
- `RootStart` 样例中带 `<Condition />`。
- `RootEnd` 在不同样例中可能带 `<Condition />`，也可能不带；为了贴近闭合样例，可为 `RootEnd` 也保留空的 `<Condition />`。
...
- 循环/遍历内部节点必须能通过 `Runners` 回到对应结束节点或后续节点，不能悬空。
...
- 先保证图上的箭头完整表达为 `Runners`，再检查每个 Tool 的 `ParentIdList` 和 `ChildrenIdList` 是否与箭头一致。
