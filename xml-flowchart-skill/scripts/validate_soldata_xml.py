...
def main():
    ...
    defined_tool_types = load_defined_tool_types()
    errors = []
    ...
    for flow in flows:
        ...
        for tool in tools.findall("Tool"):
            ...
            if not tool_id or not tool_type or tool_name is None:
                ...
                continue
            if defined_tool_types and tool_type not in defined_tool_types:
                errors.append(
                    f"Tool {tool_id} uses undefined Tool Type {tool_type}; "
                    "Type must be defined in references/main-flow-node-definitions.md "
                    "or references/node-syntax.md"
                )
